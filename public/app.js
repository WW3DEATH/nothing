...
3. Format all math and physics equations using LaTeX ($$...$$ for display equations on their own lines, and $...$ for inline math/units like $z$, $i$, $\\sqrt{-36} = 6i$).
4. Reference relevant fundamental laws (e.g. Newton's Laws, Bernoulli's Principle, Le Chatelier's Principle, Hess's Law).
5. Highlight common A/L examination pitfalls and marking scheme tips.
6. Use bullet points, bold formulas, and numbered steps for high readability.`
  },
  physics: {
...
2. Clearly state governing physics principles and standard equations formatted in LaTeX ($$...$$ for display formulas like $$F = ma$$, $$v^2 = u^2 + 2as$$, and $...$ for inline symbols).
...
2. Present structured, line-by-line algebraic proofs and geometric reasoning using LaTeX ($$...$$ for display equations like $$x = \\frac{-b \\pm \\sqrt{b^2 - 4ac}}{2a}$$, and $...$ for inline symbols like $z = 4 \\pm 6i$).
...
function formatGeminiMarkdown(text) {
  ...
  let processed = text.replace(/```([a-zA-Z0-9_-]*)\n([\s\S]*?)```/g, (match, lang, code) => {
    ...
    codeBlocks.push({ lang: lang || "CODE / FORMULA", code: code.trim() });
    return `___CODE_BLOCK_${idx}___`;
  });

  // 2. Preserve Display / Block Math: $$...$$ or \[...\]
  const mathBlocks = [];
